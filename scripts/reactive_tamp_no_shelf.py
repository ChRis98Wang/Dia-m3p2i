...
'''
运行说明：
    这个脚本启动任务规划服务器，用于无架子环境下的机械臂夹取任务
    运行命令：
    python3 reactive_tamp_no_shelf.py -cn config_panda
'''
...
class REACTIVE_TAMP:
    ...
    def tamp_interface(self):
        ...
        print("任务:", self.task_planner.task, "目标:", self.task_planner.curr_goal)
    ...
    def get_suction(self):
        # 添加类型检查，处理整数类型
        if isinstance(self.suction_active, int):
            # 如果是整数，转换为包含单个元素的张量
            return torch_to_bytes(torch.tensor([self.suction_active], device=self.cfg.mppi.device))
        else:
            # 如果已经是张量或其他类型，保持原样
